...
Session session;
ItemPurchase dataModel;
Transaction tx = null;

/********Model specific variables************/

String effectivefrom,effectiveto,itemid,uomid,supplierid,currencyid,vatid ,unitprice,minqty,maxqty;
Integer Nitemid,Nuomid,Nsupplierid,Ncurrencyid,Nvatid ;
...
dataModel = (ItemPurchase) session.get(ItemPurchase.class, id);
} else {
dataModel = new ItemPurchase();
}
...
returngridata = getItemPurchasedata(id);
...
private String getItemPurchasedata(Integer id) {
DataLoader ME = new DataLoader();
StringBuilder myquery = new StringBuilder("FROM ItemPurchase Where itemPurchaseId=" + id);
mydata = ME.getData(myquery.toString());
if (!mydata.isEmpty()) {
i = 1;
for (Iterator iterator = mydata.iterator(); iterator.hasNext();) {
ItemPurchase itempurchase = (ItemPurchase) iterator.next();
gridata = i + ","+itempurchase.getUnitPrice() + "," + itempurchase.getMinQty() + "," + itempurchase.getMaxQty() + "," + itempurchase.getItem().getItemId() + "," + itempurchase.getPrcSuppliers().getSupplierId() + "," + itempurchase.getCrmUom().getUomId() + "," + itempurchase.getFinVat().getVatId() + "," + itempurchase.getAllCurrency().getCurrencyId();
i++;
}
}
return gridata;
...
supplieridrules.put("maxlength", "30");
...
currencyidrules.put("required", "false");
currencyidrules.put("minlength", "0");
...
effectivefromrules.put("required", "false");
effectivefromrules.put("minlength", "0");
